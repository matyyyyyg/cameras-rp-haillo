"""
Hailo-8 hardware accelerated face detector.I
...
class UnifiedHailoFaceDetector:
    ...
    def _infer(self, input_data: np.ndarray) -> Dict[str, np.ndarray]:
        """
        Run inference on Hailo device using VStreams API.
        ...
        try:
            ...
            # InferVStreams handles all buffer allocation internally
            with InferVStreams(
                self._network_group,
    ...
    def _postprocess(self, outputs: Dict[str, np.ndarray], original_shape: Tuple[int, int, int]) -> List[Dict]:
        """
        Postprocess RetinaFace outputs using anchor-based decoding.

        RetinaFace outputs 9 tensors: 3 FPN levels x 3 types each.
        Each level produces classification, bbox regression, and landmark tensors.
        We identify them by their channel count:
          - 4 channels  = classification (2 anchors * 2 classes)
        ...
        try:
            ...
            for level_idx, ((feat_h, feat_w), tensors) in enumerate(sorted_levels):
                ...
                cls_data = tensors['cls']    # (H, W, 4)
                bbox_data = tensors['bbox']  # (H, W, 8)
                lm_data = tensors.get('lm')  # (H, W, 20) or None
                ...
                for anchor_idx, anchor_size in enumerate(min_sizes):
                    # Extract classification scores for this anchor
                    bg = cls_data[:, :, anchor_idx * 2]
                    fg = cls_data[:, :, anchor_idx * 2 + 1]
                    # Softmax for face confidence
                    ...
                    off = anchor_idx * 4
                    dx = bbox_data[:, :, off][mask]
                    dy = bbox_data[:, :, off + 1][mask]
                    dw = bbox_data[:, :, off + 2][mask]
                    dh = bbox_data[:, :, off + 3][mask]

                    pred_cx = cx_f + dx * variance[0] * anchor_size
                    pred_cy = cy_f + dy * variance[0] * anchor_size
                    pred_w = anchor_size * np.exp(dw * variance[1])
                    pred_h = anchor_size * np.exp(dh * variance[1])
