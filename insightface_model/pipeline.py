...
from src_old_dont_touch.kalman_tracker import KalmanPersonTracker, TrackedPerson, format_output_json
from insightface_model.classifier import InsightFaceClassifier, extract_face_crop
...
class SimplePipeline:
    ...
    def __init__(
        ...
        logger.info("Initializing Pipeline (Hailo + InsightFace + Kalman + ReID)...")
        ...
        logger.info("2/3: Loading InsightFace classifier...")
        self.classifier = InsightFaceClassifier()

        logger.info("3/3: Loading Kalman tracker with ReID...")
    ...
    def get_tracker_stats(self) -> Dict:
        return {
            ...
            "reid_gallery_size": len(self.tracker.track_gallery)
        }
...
def main():
    parser = argparse.ArgumentParser(description="InsightFace Model Pipeline")
    ...
    print("\n" + "=" * 50)
    print("INSIGHTFACE PIPELINE (ReID ON) - Press 'q' to quit")
    print("=" * 50 + "\n")
    ...
    try:
        while True:
            ...
            if args.display:
                ...
                cv2.putText(annotated, f"Tracking: {stats['active_tracks']} (Kalman+ReID)", (10, 50),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                cv2.putText(annotated, f"Gallery: {stats['reid_gallery_size']}", (10, 75),
                ...
                cv2.imshow('InsightFace Pipeline (ReID ON)', annotated)
            ...
            if frame_count % 30 == 0:
                ...
                logger.info(
                    ...
                    f"FPS: {avg_fps:.1f}, "
                    f"Gallery: {stats['reid_gallery_size']}, "
        ...
        print(f"ReID gallery size: {stats['reid_gallery_size']}")
